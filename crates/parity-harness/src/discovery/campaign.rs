...
impl AdmissionQueue {
    ...
    ///
    /// Two rules, both US1's and both preserved verbatim by the extraction:
    ///
    ...
    ///
    /// Note what the cap is measured against: `admitted`, which holds every id this campaign
    /// *touched*, re-witnesses included. A campaign that re-witnesses `admission_cap`
    /// standing findings therefore has no budget left for a new one. That is deliberate on
    /// the reviewer-throughput reading of the cap and is the behavior US1 shipped and tested;
    /// changing it is a decision about the cap's meaning, not a refactor.
    fn offer(&mut self, signature: Signature, witness: Witness) {
        ...
        if !already_known && self.admitted.len() as u64 >= self.admission_cap {
