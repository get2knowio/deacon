...
fn test_runtime_env_var_invalid_fallback() -> Result<()> {
    ...
    let mut cmd = Command::cargo_bin("deacon")?;
    cmd.env("DEACON_RUNTIME", "invalid")
       .args(&["up", "--help"]);
    cmd.assert().success();
    Ok(())
}
...
fn test_up_command_with_podman_runtime_error() -> Result<()> {
    ...
    let devcontainer_path = temp_dir.path().join(".devcontainer").join("devcontainer.json");
    std::fs::create_dir_all(devcontainer_path.parent().unwrap())?;
    std::fs::write(&devcontainer_path, r#"{"image": "mcr.microsoft.com/devcontainers/base:ubuntu"}"#)?;
