...
"#;
    ...
    cmd.current_dir(&temp_dir)
        .args(&[
            "--config", base_config.to_str().unwrap(),
            "--override-config", override_config.to_str().unwrap(), 
            "--secrets-file", secrets_file.to_str().unwrap(),
            "read-configuration"
        ]);
...
fn test_cli_with_multiple_secrets_files() {
    ...
    cmd.current_dir(&temp_dir)
        .args(&[
            "--config", base_config.to_str().unwrap(),
            "--secrets-file", secrets1.to_str().unwrap(),
            "--secrets-file", secrets2.to_str().unwrap(),
            "read-configuration"
        ]);
...
fn test_cli_missing_secrets_file_continues() {
    ...
    cmd.current_dir(&temp_dir)
        .args(&[
            "--config", base_config.to_str().unwrap(),
            "--secrets-file", missing_secrets.to_str().unwrap(),
            "read-configuration"
        ]);
