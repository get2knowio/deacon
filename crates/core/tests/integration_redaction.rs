...
use deacon_core::{
    ...
    redaction::{global_registry, add_global_secret, RedactionConfig, redact_if_enabled},
};
use serde_json::json;
...
fn test_redaction_in_lifecycle_execution() {
    ...
    let commands_json = json!(["echo 'The password is my-secret-password'", "echo 'No secrets here'"]);
    let env = HashMap::new();
    ...
    let ctx = ExecutionContext::new()
        .with_redaction_config(RedactionConfig::default());
...
fn test_redaction_disabled_shows_secrets() {
    ...
    let ctx = ExecutionContext::new()
        .with_redaction_config(RedactionConfig::disabled());
...
fn test_partial_secret_matches_not_redacted() {
    ...
    // Create commands with partial matches
    let commands_json = json!(["echo 'This has super and secret but not the full supersecret'"]);
    let env = HashMap::new();
    let commands = LifecycleCommands::from_json_value(&commands_json, &env).unwrap();
...
fn test_redaction_performance_short_lines() {
    ...
    // Performance should be reasonable - less than 100ms for 5000 operations
    // This is a rough benchmark to ensure redaction doesn't add significant overhead
    assert!(duration.as_millis() < 100, "Redaction took too long: {:?}", duration);
...
fn test_redaction_performance_with_secrets() {
    ...
    assert!(duration.as_millis() < 200, "Redaction with secrets took too long: {:?}", duration);
