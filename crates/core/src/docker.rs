...
impl ContainerOps for CliDocker {
    ...
    async fn create_container(
        ...
        let image = config.image.as_ref()
            .ok_or_else(|| DockerError::CLIError("No image specified in configuration".to_string()))?;
        ...
        let container_id = tokio::task::spawn_blocking(move || {
            ...
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                return Err(DockerError::CLIError(format!("Docker create failed: {}", stderr)));
            }
    ...
    async fn start_container(&self, container_id: &str) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || -> std::result::Result<(), DockerError> {
            ...
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                return Err(DockerError::CLIError(format!("Docker start failed: {}", stderr)));
            }
    ...
    async fn remove_container(&self, container_id: &str) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || -> std::result::Result<(), DockerError> {
            ...
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                return Err(DockerError::CLIError(format!("Docker rm failed: {}", stderr)));
            }
    ...
    async fn get_container_image(&self, container_id: &str) -> Result<String> {
        ...
        tokio::task::spawn_blocking(move || -> std::result::Result<String, DockerError> {
            ...
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                return Err(DockerError::CLIError(format!("Docker inspect failed: {}", stderr)));
            }
...
impl DockerLifecycle for CliDocker {
    ...
    async fn up(
        ...
        debug!("Successfully created and started new container: {}", container_id);
