...
test-nextest-ci: ## Run CI test suite with cargo-nextest (conservative concurrency with timing capture)
	@set -euo pipefail; \
	./scripts/nextest/assert-installed.sh; \
	mkdir -p artifacts/nextest; \
	echo "Running nextest with ci profile..."; \
	start_time=$$(date +%s); \
	if cargo nextest run --profile ci $(THREAD_ARGS) --success-output never --failure-output immediate --show-progress none; then \
		end_time=$$(date +%s); \
		duration=$$((end_time - start_time)); \
		timestamp=$$(date -u +"%Y-%m-%dT%H:%M:%SZ"); \
		...
		# Generate runtime comparison if baseline exists \
