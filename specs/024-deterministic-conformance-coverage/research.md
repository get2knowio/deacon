...
| Activating a second environment profile | Assumption 10. FR-004b keeps activation a data change, so deferring costs nothing structurally. **T149 outcome: modelled, attempted, deliberately not activated.** `prof-linux-amd64-podman-0870` now exists with `active: false`. Activation was tried for real and produced results byte-identical to the docker profile (787 obligations, 370 covered, 417 gap, `inactive-environment 0`), because all 55 behaviors have an empty `applicability` and so apply everywhere — the swap moves nothing, verifies nothing, and would claim the registry's `reference` axis holds under a runtime no case has run against. Real activation needs a podman execution lane plus runtime-conditioned applicability on the behaviors that genuinely differ. |
