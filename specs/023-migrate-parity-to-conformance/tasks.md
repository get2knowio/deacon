...
- [X] T107 [Deferral, US2 residual `res-exec-per-side-argv`] Give a declarative operation per-side argument vectors, then migrate `parity_exec::env-propagation` — **RESOLVED: the premise was false**
  ...
  - **Resolved (024 Phase 6)** by the SECOND branch of that acceptance, and no runner capability was added. The flags had already converged: `cli.rs:534` declares `--remote-env` as the primary flag with `alias = "env"` for the legacy spelling, so the single shared argv `--remote-env FOO=BAR` runs on both CLIs and the comparison is a true differential. The residual was recording a constraint that no longer existed rather than one still pending — which is exactly what makes a self-invalidating record worth having, and what makes an un-revisited one dangerous. `case-exec-decl-env-propagation` landed, `res-exec-per-side-argv` is deleted, and per-side argv was NOT built (it would have let a case compare two different commands and call the result parity).
...
- [X] T110 [Deferral, US3 T054] Give `bhv-exec-container-id-metadata` its own independently-reported case — **DONE via the `${CONTAINER_ID}` argv token**
  ...
  - **Resolved (024 Phase 6)**: `runner::substitute_argv` gained `${CONTAINER_ID}`, which resolves to the container the most recent successful `up`/`exec` op produced. The op loop already tracked `container_id`; threading it in was the whole change. Deliberately NOT a new `Operation` field — the token is a string inside the existing `argv`, so the operation model is unchanged and every existing `caseHash` is byte-identical, which is why no snapshot needed re-recording. A token with no preceding successful `up` fails LOUD, the same discipline `${WORKSPACE}`-without-a-fixture has: handing the literal `${CONTAINER_ID}` to the CLI would make the op fail for an unrelated reason and read as a real divergence.
  - `case-exec-decl-container-id-metadata` runs `up` then `exec --container-id ${CONTAINER_ID}` printing `$REMOTE_ONLY`, a value the fixture declares in devcontainer.json and NOWHERE else — so a byte-identical result on both sides proves each CLI recovered it from the label it stamped, rather than from a workspace it re-read. Verified `agree` on the live differential against pinned oracle 0.87.0. `case-up-exec-parity` is deleted along with its carrier, so the two-behaviors-one-outcome claim is gone rather than merely annotated.
...
- [ ] T115 [Deferral, 024 Phase 5] deacon stamps SUBSTITUTED paths into `devcontainer.metadata`; the reference stamps the template
