...
- [ ] T034 [Foundational] Decide and enforce `--permit-randomization` behavior
...
- [ ] T029 [US2] Enforce duplicate/idempotence disabled in `--global-scenarios-only`
...
- [ ] T031 [US2] Explicit zero-tests branch (text + JSON)
   - Files: `crates/deacon/src/commands/features.rs` — when selection/filter yields zero tests, print "No tests found" (text mode) and exit 0; with `--json`, stdout is exactly `[]`
   - Tests: `crates/deacon/tests/integration_features_test.rs` asserts text summary and exact JSON array
- [ ] T032 [US2] Default base image when `--base-image` is absent
   - Files: `crates/core/src/features_test/runner.rs` — default to `ubuntu:focal` unless overridden; honor `--remote-user`
...
- [ ] T030 [P] [US3] Cross-platform path normalization & discovery consistency
   - Files: `crates/core/src/features_test/discovery.rs` — normalize path separators, maintain case-sensitive discovery, support spaces in paths and feature IDs
   - Tests: `crates/core/tests/features_test_paths.rs` for normalization and discovery with mixed separators and spaces
- [ ] T033 [P] [US3] Pin scenario file location & minimal schema + validation
   - Files: `crates/core/src/features_test/discovery.rs` — load `test/<feature>/scenarios.json` or `.jsonc`; on parse error, include file path (and line/col when available)
...
- [ ] T035 [P] Normalize terminology to "duplicate/idempotence" across docs and user-facing output
