...
- [ ] T051 Verify the certification lane on the feature branch: push, confirm `parity / live-certification` triggers via its `paths:` filter, runs green (or red for characterized reasons with artifacts uploaded), and `ci.yml` lanes remain green — the US3/SC-007 truthful-status proof in situ
